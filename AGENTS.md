...
│   │   ├── integrations/page.tsx     # /integrations — connect/manage integrations (Slack, Email, SMS)
...
│   │   ├── integrations/email/route.ts  # POST — connect/edit the email integration (no OAuth, just recipient addresses; upsert doubles as edit)
│   │   ├── integrations/sms/route.ts    # POST — connect/edit the sms integration (recipient phones + notify_impacts severity filter)
...
│   ├── notifyIncidentEvents.ts       # notifyPendingEvents() — Slack + email + SMS notification fan-out over incident_events
