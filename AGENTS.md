...
- **Data layer**: Supabase (Postgres). There is no standalone "tracked services" table anymore — tracking a service *is* adding it to one of your boards (`boards.service_slugs`, a plain array; the same service can sit on several of your own boards at once). The old global `services` table was retired in `0017_boards_own_tracking.sql` for exactly this reason: it was one shared list with no owner, incompatible with a real multi-tenant product. The fixed catalog of known services (previously an in-code `SERVICE_CATALOG` constant) is a `catalog` table, read via `lib/catalog.ts`'s `getCatalog()`/`resolveCatalogEntryBySlug()`, written via `ensureCatalogEntry()` (SSRF-validated — see Security), and reseeded with `npm run import:catalog` (`scripts/import-catalog.mjs`). Both `boards` and `integrations` are per-account with real RLS (`0013_board_ownership.sql`, `0018_integration_ownership.sql`) — see the RBAC note below, this is no longer a single shared workspace. Incident history for the 1-minute poller lives in `incidents`/`incident_updates`/`incident_events`/`incident_event_deliveries` (see `supabase/migrations/`); maintenance history follows the same shape via `lib/getStoredMaintenance.ts`. Live status itself is still never stored server-side beyond that — it's fetched from each tracked host's public Atlassian Statuspage JSON endpoints on every request/poll (`revalidate: 60`)
  - **Billing** (`0022_create_subscriptions.sql`): one `subscriptions` row per account (`lib/subscriptions.ts`), synced from Stripe. Unlike `boards`/`integrations`, this table is **select-only** for the client — RLS has no insert/update/delete policy for `authenticated` at all, so a signed-in user can never write their own `plan`/`status` directly. Every write (checkout fulfillment, cancel/resume, webhook sync) goes through `lib/subscriptions.ts`'s service-role write helpers, each narrowly scoped and comment-justified. No row means the free tier, the same "absence = not connected" convention `integrations` uses. Tier/price metadata (`lib/plans.ts`) is the single source shared by the public pricing page and the dashboard billing page; actual Stripe Price ids live only in env vars (`lib/stripe.ts`'s `resolvePriceId()`), never in `lib/plans.ts` itself since that file is imported by client components
...
│   │   ├── billing/subscription/route.ts, billing/checkout/route.ts, billing/cancel/route.ts  # GET current subscription; POST create a Checkout Session; POST cancel/resume
│   │   ├── billing/webhook/route.ts  # POST — public (proxy.ts PUBLIC_EXACT), Stripe signature-verified, syncs `subscriptions` from customer.subscription.* events
...
│   ├── subscriptions.ts              # Supabase `subscriptions` CRUD — getSubscription/getStripeCustomerId/getOwnStripeSubscriptionId (session-scoped reads) / upsertFromStripeEvent/applyCancelState (service-role writes, see Data layer)
│   ├── stripe.ts                     # getStripeClient() — lazy, server-only, Stripe secret key; resolvePriceId/resolvePlanFromPriceId (tier×interval ↔ env-var Price id)
│   ├── plans.ts                      # PLAN_CATALOG — shared tier/price/feature data for the pricing page and the billing page (client-safe, no Stripe ids)
...
├── types/                            # service.ts, board.ts, integration.ts, subscription.ts, logo.ts, i18n.ts — each imported directly (`@/types/service`, etc.), there is no barrel `index.ts`
...
- `GET /api/integrations/email/verify` is deliberately public (`proxy.ts`'s `PUBLIC_EXACT`) — it's clicked from an email client, which may have no session for the account that added the recipient at all, so the verification token itself is the authorization, not a session check. Any future "confirm via a link sent externally" flow needs the same treatment: public route, secret-bearing token, not session-gated. `POST /api/billing/webhook` is the same idea from a different caller: Stripe carries no session cookie either, so it's public in `PUBLIC_EXACT` too, and `stripe.webhooks.constructEvent()`'s signature check is the authorization, not `CRON_SECRET`-style manual `timingSafeEqual` — the Stripe SDK does its own constant-time verification internally
- Real per-account data now exists (`boards`, `integrations`, `subscriptions`, all RLS'd) that a malicious/careless client-supplied id could otherwise let one account read or mutate another's — every new route touching any of them must go through the session-scoped client (`lib/boards.ts`/`lib/integrations.ts`/`lib/subscriptions.ts`'s normal read functions) so RLS does the scoping, not a hand-rolled `.eq("user_id", ...)` that's easy to forget on one code path. `subscriptions` goes further: it has no client-facing write path at all (see Data layer) — a `plan`/`status` column must never become updatable by a raw authenticated request, only by server code that has independently verified the change against Stripe
...
- `proxy.ts`'s logged-out redirect only ever preserved the pathname in `next` (`loginUrl.searchParams.set("next", pathname)`), silently dropping any query string. Invisible for routes with no meaningful query params, but it broke the billing flow: a landing-page "Get started" link to `/billing?plan=pro&interval=month` would bounce an anonymous visitor through `/login` and land them back on bare `/billing`, losing which plan they meant to buy. Fixed by appending `request.nextUrl.search` to the preserved path. Any new feature that links to a protected route with query params should not assume they'll survive an anonymous visit without checking this.
